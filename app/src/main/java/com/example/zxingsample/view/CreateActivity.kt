...
//                )
    ...
    private fun saveQr(contentResolver: ContentResolver, bitmap: Bitmap) {
        val contentValues = ContentValues().apply {
            put(MediaStore.Images.Media.DISPLAY_NAME, System.currentTimeMillis()) // 파일 이름
            put(MediaStore.Images.Media.MIME_TYPE, "image/png") // MIME 타입 (PNG 이미지)
            put(MediaStore.Images.Media.RELATIVE_PATH, "Pictures/QR Code") // 상대 경로
            put(MediaStore.Images.Media.IS_PENDING, 1) // 저장 중 상태 설정
        }
